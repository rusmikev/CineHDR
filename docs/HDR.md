...
* **Backend-specific reality:** CineHDR delegates OpenGL rendering to mpv's Render API. The standard `opengl` backend still uses the legacy GPU renderer, restores pre-Dolby-Vision signaling, and does not process RPU metadata. The experimental, pinned custom libmpv additionally exposes `opengl-next`; source review and a rendered-pixel negative control show that its libplacebo path can consume mapped RPU metadata for the exact pinned dependency stack. This is path/effect evidence only: color accuracy, Profile 5, subtitles, hardware coverage, enhancement-layer/FEL reconstruction, and native Dolby Vision output remain unvalidated. It must not be described as full Dolby Vision, passthrough, or certification.
* **Profile 7 & 8 (`Base Layer + RPU`):** Legacy `opengl` plays a usable PQ base layer where one exists and bypasses the RPU. Pinned `opengl-next` may apply mapped RPU reshaping to the rendered Rec.2100 PQ pixels, but that result remains experimental with color validation pending. Profile 8 alone does not prove an HDR10-compatible base layer because CineHDR cannot currently read `dv_bl_signal_compatibility_id`; diagnostics must not label every Profile 8 stream as HDR10 fallback. Profile 7 enhancement-layer/FEL reconstruction remains unsupported on both backends.
* **Profile 5 (`IPTPQc2` Proprietary Color Space):** Attaching a `Rec.2100 PQ` color state to unshaped Profile 5 IPT frames yields false colors (green/purple tint) *and* flips the monitor into HDR mode. Note that content detection alone cannot prevent this: libplacebo's `pl_map_avdovi_metadata()` rewrites the decoder-side frame parameters to `primaries=bt.2020` / `transfer=pq` for every single-layer DoVi stream, so `video-params` reports `gamma="pq"` and `is_hdr_content()` returns `True` for Profile 5 as well. The refusal therefore lives in `HdrController.is_hdr_active`, as a **capability gate placed ahead of the user's `hdr-mode`** (`DOVI_UNSUPPORTED_PROFILES = (5,)`): Profile 5 always falls back to mpv's SDR tone mapping, including under experimental `opengl-next`, and even `force-hdr` cannot override it. The gate may be reconsidered only after ADR-0001 Gate 3 passes with an explicit version-qualified capability; backend name alone is insufficient. A one-shot warning is logged (`check_dovi_warning()`).
...
* **GraphicsOffload:** `Window` wraps `MpvVideoWidget` inside `Gtk.GraphicsOffload`. Direct scanout via `GraphicsOffload` is controlled by the GSettings `graphics-offload` key (default `false`). When disabled, video is composited normally through GTK's scene graph without direct scanout.

### E. Compositor Capability Probe (`wayland_cm_probe.py`)
`check_hdr_support()` used to trust proxy signals only (Wayland session, GTK API presence, dmabuf formats). None of those prove the *compositor* can accept a Rec.2100 PQ surface: on a Wayland compositor without color management GTK silently converts PQ -> sRGB with a plain colorimetric transform, which looks *worse* than mpv's tone mapping — the exact "washed out unless you pick Force SDR" failure mode described in the README. The registry probe adds a necessary compositor precondition, not sufficient surface-submission evidence.
* **Mechanism:** a private `wl_event_queue` + proxy wrapper is attached to GTK's own `wl_display` (the documented libwayland pattern for sharing a connection with a toolkit), the registry globals are enumerated in one round-trip, and the result is reduced to a tri-state:
  * `True` — `wp_color_manager_v1` (ratified protocol; KWin / Plasma 6.3+, Mutter / GNOME 48+) or `xx_color_manager_v4` (experimental predecessor spoken by GTK 4.16/4.17) is advertised. This means only that negotiation may be possible.
  ...
  * `None` — the probe could not run (no Wayland display, libwayland unavailable). `None` neither blocks nor allows HDR by itself: the decision passes to the GTK color-management gate (`GDK_DEBUG=color-mgmt` opt-in plus compositor capabilities from the `wayland_output_hdr` probe, invariant G), which **fails closed**: unknown capabilities refuse HDR pass-through and mpv tone mapping is used. The typical causes of `None` also break the capability probe, so in practice `None` means SDR tone mapping. (Changed in ee2d333: before it, an inconclusive probe kept the older heuristic and could hand a PQ surface to a non-color-managed GTK, the washed-out failure mode above.)
...
* **Measured limitation:** the Gate 2 passive trace on GTK 4.22.4/KWin 6.7.4 found the global and a successful manager capability burst, but GTK destroyed its manager without `get_surface` or `set_image_description`, even with the documented experimental `GDK_DEBUG=color-mgmt` opt-in. Therefore the current `True` result must not be described as proof of native HDR surface submission. Product fallback policy for this newly measured gap remains a separate decision; normal playback behavior is unchanged in this development slice.
* **Opt-in timing:** GDK reads `GDK_DEBUG` exactly once, in `gdk_pre_parse()` during `Gtk.init()`, and PyGObject runs `Gtk.init_check()` as soon as `gi.repository.Gtk` is imported. The opt-in (`CINEHDR_EXPERIMENTAL_COLOR_MGMT=1`, `--experimental-color-mgmt`) is therefore applied by the launchers (`cinehdr`, `run_dev.py`) before any GTK import (`gtk_cm_policy.apply_color_mgmt_opt_in`). `main.py` only verifies it and never rewrites `GDK_DEBUG` after GTK is initialized, so `gtk_cm_opted_in()` always describes what GTK actually parsed; it mirrors GDK's parser (separators `:;, \t`, case-insensitive, `all` inverts the listed flags, so `all:color-mgmt` disables color management).
...
* **Mechanism:** on the same private event queue, `wp_color_manager_v1` is bound from the registry and its capability burst (`supported_intent`, `supported_feature`, `supported_tf_named`, `supported_primaries_named`) is parsed into `CmCaps` for GTK color-management validation (`gtk_cm_policy.py`); for every `GdkMonitor`, GTK's own `wl_output` proxy (`gdk_wayland_monitor_get_wl_output()`) is passed to `get_output` -> `get_image_description` -> `get_information`, and `tf_named` / `primaries_named` / `luminances` (including `target_luminance` and `target_max_cll`) are collected until `done`. The `wl_interface`/`wl_message` tables for the four `wp_*` interfaces are hand-built in ctypes (libwayland dispatches foreign-interface events through libffi using exactly these tables — the event rows are locked by `test_interface_tables_match_protocol`). The `icc_file` fd is closed immediately; ownership transfers with the event.
...
| **Display HDR Supported** | Whether the local GTK/Wayland high-precision prerequisites and a compositor color-manager global are present (`check_hdr_support()`). | This is a capability preflight, not proof that GTK submitted a color-managed surface. Reports `No` if running under X11, `GSK_RENDERER=gl`, or GTK lacks the required APIs. |
...
| **Compositor Color Management** | Direct registry answer (`wayland_cm_probe.probe_color_management()`): is `wp_color_manager_v1` / `xx_color_manager_v4` advertised? | `Yes` is necessary but does not prove GTK surface submission; `No` means the required compositor protocol is absent, and `Unknown` means the probe could not run. |
...
| **Dolby Vision Profile** | Reports detected Dolby Vision metadata (`current-tracks/video/dolby-vision-profile` or colormatrix) separately from renderer capability and validation status. | Standard `opengl` reports an unprocessed RPU/base-layer fallback where valid. The exact pinned `opengl-next` stack may report `RPU metadata mapped ... validation pending`; this is not a support, accuracy, passthrough, native-DV, or FEL claim. Profile 5 remains forced to SDR. |
