...
* **Wayland Color Management Pipeline**: CineHDR prepares a true 16-bit `Rec.2100 Linear`/`PQ` surface and communicates target color states to GTK4. End-to-end native HDR output to the screen depends on the ongoing Wayland color-management protocol implementation in GTK and the compositor:
  * **KDE Plasma (KWin)**: Currently, KWin advertises HDR profiles but does not advertise the `sRGB` transfer function in its Wayland color management capabilities. GTK 4 strictly requires `sRGB` and will abruptly disable all color management if it's missing. When this happens, CineHDR detects the missing capability and automatically falls back to high-quality SDR tone-mapping via `mpv` to prevent colors from washing out. True HDR on KDE requires a fix in KWin or GTK 4.
  * **GNOME (Mutter)**: GTK 4 color management works correctly on GNOME. To enable it, you must activate the experimental HDR feature in Mutter (`gsettings set org.gnome.mutter experimental-features "['experimental-hdr']"`) and toggle HDR in the GNOME Display settings.
