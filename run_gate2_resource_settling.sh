...
#
# Explicit Gate 2 bounded resource-settling validation. This is local-only:
# it neither installs software nor selects a graphics device.

set -euo pipefail

script_dir="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd -P)"
fixture_cache="$(mktemp -d /tmp/cinehdr-resource-settling-fixtures.XXXXXX)"
...
if [[ -z "${WAYLAND_DISPLAY:-}" ]]; then
    echo "CineHDR resource-settling validation requires a Wayland session." >&2
    exit 1
fi
if [[ ! -e "${gpu_next_lib}" ]]; then
    echo "CineHDR resource-settling validation: pinned libmpv is missing: ${gpu_next_lib}" >&2
    exit 1
fi
...
# Obtain media paths from the generator's recorded metadata. Cache filenames
# and globs are not fixture provenance.
...
if [[ ${#fixture_paths[@]} -ne 2 ]]; then
    echo "CineHDR resource-settling validation: fixture provenance returned invalid paths." >&2
    exit 1
fi
...
if [[ "$(sha256sum -- "${sdr_media}" | awk '{print $1}')" != "${sdr_sha256}" ||
    ...
    echo "CineHDR resource-settling validation: generated media SHA-256 mismatch." >&2
    exit 1
fi

run_backend() {
    local backend="$1"
    local log_path="${run_dir}/resource-settling-${backend}.log"
    local marker
    ...
    echo "CineHDR resource-settling validation (${backend}): ${log_path}"
    process_env=(
        "CINEHDR_RENDER_BACKEND=${backend}"
        "CINEHDR_GPU_VALIDATION=settling"
    ...
    if [[ "${application_status}" -ne 0 ]]; then
        echo "CineHDR resource-settling validation (${backend}) exited ${application_status}." >&2
        exit "${application_status}"
    fi
    mapfile -t reports < <(
        find "${run_dir}" -maxdepth 1 -type f \
            -name "gate2-resource-settling-${backend}-*.json" \
            -newer "${marker}" -print
    )
    if [[ ${#reports[@]} -ne 1 ]]; then
        echo "CineHDR resource-settling validation (${backend}) expected one fresh report, found ${#reports[@]}." >&2
        exit 1
    fi
    python3 - "${reports[0]}" "${backend}" <<'PY'
import json
from pathlib import Path
import sys
...
backend = sys.argv[2]
if (report.get("schema") != "cinehdr.gate2.resource-settling.v1" or
        report.get("revision") != "bounded-resource-settling-v3" or
        report.get("expected_backend") != backend or
        report.get("status") not in {"PASS", "WARN"}):
    raise SystemExit("resource-settling report was not a strict PASS/WARN record")
PY
    echo "CineHDR resource-settling report (${backend}): ${reports[0]}"
    rm -f -- "${marker}"
}

# Separate processes are mandatory; the renderer API is startup-selected.
run_backend legacy
